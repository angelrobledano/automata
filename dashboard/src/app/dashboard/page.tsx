    ...
    return (
      <div className="flex h-screen items-center justify-center bg-gray-50">
        <div className="flex flex-col items-center">
          ...
          <p className="text-gray-500 font-medium">Calculando el impacto de tu IA...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-gray-50">
        <div className="text-center">
          <p className="text-gray-900 font-medium mb-2">No hemos podido cargar los datos.</p>
          <p className="text-gray-500 text-sm">Actualiza la página en unos instantes.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto py-6 px-6 font-sans pb-12">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Resumen de Rendimiento</h1>
          <p className="text-gray-500 mt-1">Aquí tienes el impacto de tu asistente calculado en tiempo real.</p>
        </div>
        <div className="flex items-center gap-4">
          <select 
            value={period}
            onChange={(e) => setPeriod(e.target.value)}
            className="bg-white border border-gray-200 text-gray-700 text-sm font-semibold rounded-lg px-3 py-1.5 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none shadow-sm cursor-pointer"
          >
      ...
      {metrics?.insights && metrics.insights.length > 0 && (
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-lg font-bold text-gray-900">Insights Predictivos</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {metrics.insights.map((insight: any) => (
              <div key={insight.id} className="bg-gradient-to-br from-indigo-50 to-white border border-indigo-100 rounded-2xl p-5 shadow-sm relative overflow-hidden group">
                ...
                <h3 className="font-bold text-gray-900 mb-2 relative z-10">{insight.title}</h3>
                <p className="text-sm text-gray-600 mb-5 relative z-10">{insight.description}</p>
                {insight.actionLabel && (
                  <button 
                    onClick={() => handleResolveInsight(insight)}
                    disabled={resolvingInsight === insight.id}
                    className="relative z-10 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-xs font-bold px-4 py-2 rounded-xl transition-colors shadow-sm flex items-center gap-2"
                  >
      ...
      <motion.div 
        ...
        className="relative overflow-hidden bg-indigo-900 rounded-3xl p-6 mb-6 shadow-xl border border-indigo-800/50"
      >
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <motion.div 
              ...
              className="bg-indigo-800/50 backdrop-blur-sm border border-indigo-700/50 hover:border-indigo-500/80 hover:shadow-[0_0_15px_rgba(99,102,241,0.5)] transition-all rounded-2xl p-4 cursor-default"
            >
            ...
            <motion.div 
              whileHover={{ scale: 1.02, y: -2 }}
              className="bg-indigo-800/50 backdrop-blur-sm border border-indigo-700/50 hover:border-amber-500/80 hover:shadow-[0_0_15px_rgba(245,158,11,0.5)] transition-all rounded-2xl p-4 cursor-default"
            >
            ...
            <motion.div 
              whileHover={{ scale: 1.02, y: -2 }}
              className="bg-indigo-800/50 backdrop-blur-sm border border-indigo-700/50 hover:border-green-400/80 hover:shadow-[0_0_15px_rgba(74,222,128,0.5)] transition-all rounded-2xl p-4 cursor-default"
            >
      ...
      >
        ...
        <motion.div whileHover={{ y: -4 }} className="bg-white rounded-3xl p-5 border border-gray-100 shadow-sm hover:shadow-lg transition-all flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start mb-1 group relative">
              <p className="text-xs font-semibold text-gray-500 flex items-center gap-1 cursor-help">
              ...
              <div className="absolute top-6 left-0 bg-gray-900 text-white text-xs p-3 rounded-xl w-48 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20 shadow-xl">
            ...
            <h4 className="text-4xl font-extrabold text-gray-900 tracking-tight mb-1">{metrics.moneySaved} €</h4>
            <p className="text-[10px] text-gray-500">Basado en un coste operativo de 15€/hora.</p>
          </div>
          <div className="mt-4 pt-3 border-t border-gray-50">
            <p className="text-xs text-gray-600 font-medium">El valor del tiempo que tu equipo ahorró en teclear.</p>
          </div>
        </motion.div>

        <motion.div whileHover={{ y: -4 }} className="bg-white rounded-3xl p-5 border border-gray-100 shadow-sm hover:shadow-lg transition-all flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <h4 className="text-4xl font-extrabold text-indigo-600 tracking-tight mb-1">{metrics.automationRate}%</h4>
            <p className="text-[10px] text-gray-500">De {metrics.totalConversations} conversaciones totales.</p>
          </div>
          <div className="mt-4 pt-3 border-t border-gray-50 relative z-10">
            <p className="text-xs text-gray-600 font-medium">Clientes que no necesitaron ayuda de un humano.</p>
          </div>
        </motion.div>
        ...
        <motion.div whileHover={{ y: -4 }} className="bg-white rounded-3xl p-5 border border-gray-100 shadow-sm hover:shadow-lg transition-all flex flex-col justify-between">
          <div>
            ...
            <h4 className="text-4xl font-extrabold text-gray-900 tracking-tight mb-1">{metrics.salesGenerated} €</h4>
            <p className="text-[10px] text-gray-500">Basado en {metrics.aiResolvedConversations} chats resueltos.</p>
          </div>
          <div className="mt-4 pt-3 border-t border-gray-50">
            <p className="text-xs text-gray-600 font-medium">Valor retenido por responder al instante.</p>
          </div>
        </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h4 className="text-sm font-bold text-gray-900">Volumen de Mensajes (Últimos 7 días)</h4>
              <p className="text-[10px] text-gray-500">Mensajes recibidos vs respondidos por IA.</p>
            </div>
            <div className="flex gap-3">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-medium text-gray-600">Por IA</span>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm flex flex-col justify-between">
          <div>
            <h4 className="text-sm font-bold text-gray-900 mb-4">Salud del Asistente</h4>
          ...
          <div className="mt-4 bg-gray-50 rounded-xl p-3 border border-gray-100 text-center">
            <p className="text-[10px] text-gray-500 mb-0.5">Tiempo de Respuesta IA</p>
            <p className="text-lg font-extrabold text-gray-900">1.2 seg</p>
