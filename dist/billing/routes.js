...
const PaymentEngine_1 = require("./core/PaymentEngine");
const prisma_1 = require("../db/prisma");
const router = (0, express_1.Router)();
const engine = new PaymentEngine_1.PaymentEngine();
// --- WEBHOOK ENDPOINT ---
...
router.post('/webhook', express_2.default.raw({ type: 'application/json' }), async (req, res) => {
    ...
    try {
        const event = await engine.processWebhook(req.body, signature);
        // Idempotency check
        const isNew = await prisma_1.prisma.billingEvent.findUnique({ where: { providerEventId: event.eventId } });
        if (isNew) {
...
router.get('/plans', async (req, res) => {
    const plans = await prisma_1.prisma.plan.findMany({ include: { features: true } });
    res.json(plans);
});
...
router.put('/plans/:id', async (req, res) => {
    ...
    // Recreate features
    await prisma_1.prisma.planFeature.deleteMany({ where: { planId: req.params.id } });
    if (data.features && data.features.length > 0) {
        await prisma_1.prisma.planFeature.createMany({
            data: data.features.map((f) => ({ ...f, planId: req.params.id }))
        });
    }
...
router.get('/portal-data', async (req, res) => {
    const commerceId = req.headers['x-commerce-id']; // Usually from Auth Middleware
    const plans = await prisma_1.prisma.plan.findMany({ where: { status: 'ACTIVE' }, include: { features: true } });
    const commerce = commerceId ? await prisma_1.prisma.commerce.findUnique({
        where: { id: commerceId },
...
router.post('/checkout', async (req, res) => {
    const commerceId = req.headers['x-commerce-id'];
    const { planId, interval } = req.body;
    const commerce = await prisma_1.prisma.commerce.findUnique({ where: { id: commerceId } });
    ...
    if (!customerId) {
        ...
        await prisma_1.prisma.commerce.update({ where: { id: commerce.id }, data: { billingCustomerId: customerId } });
    }
    const session = await engine.createSubscriptionCheckout(customerId, plan.providerPriceId, `${process.env.DASHBOARD_URL}/ajustes/billing?success=true`, `${process.env.DASHBOARD_URL}/ajustes/billing?canceled=true`);
    res.json({ url: session.url });
});
router.put('/preferences', async (req, res) => {
    const commerceId = req.headers['x-commerce-id'];
    const { overageBehavior } = req.body;
    await prisma_1.prisma.commerce.update({
        where: { id: commerceId },
