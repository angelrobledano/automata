...
class FeatureGuard {
     ...
     */
    static async canExecute(commerceId, featureKey, requestedAmount = 1) {
     ...
     */
    static async trackConsumption(commerceId, metricKey, amount = 1) {
        const activeSub = await prisma.subscription.findFirst({
            where: { commerceId, status: 'ACTIVE' }
        });
        if (!activeSub)
            return; // Silent return or throw depending on strictness
        const now = new Date();
