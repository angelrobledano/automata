...
exports.detectIntentAndContext = detectIntentAndContext;
exports.resolveApplicableFacts = resolveApplicableFacts;
const prisma_1 = require("../db/prisma");
/**
 * 1. CLASIFICACIÓN DE INTENCIÓN Y CONTEXTO TEMPORAL
 */
function detectIntentAndContext(userQuestion, referenceDate = new Date()) {
 ...
 */
async function resolveApplicableFacts(commerceId, userQuestion, referenceDate = new Date()) {
    ...
    let rules = await prisma_1.prisma.structuredKnowledgeRule.findMany({
        where: {
            commerceId,
            type: intent === 'BUSINESS_HOURS' ? { in: ['BUSINESS_HOURS', 'HOLIDAY_CLOSURE'] } : intent
        },
    ...
    for (const rule of rules) {
        ...
        if (rule.validFrom && targetDate < new Date(rule.validFrom))
            continue;
        if (rule.validUntil && targetDate > new Date(rule.validUntil))
            continue;
        if (rule.monthsOfYear && rule.monthsOfYear.length > 0) {
            if (!rule.monthsOfYear.includes(monthNumber))
                continue;
        }
        if (rule.daysOfWeek && rule.daysOfWeek.length > 0) {
            if (!rule.daysOfWeek.includes(dayOfWeekNumber))
                continue;
        }
        matchingRules.push(rule);
    }
    ...
    if (activeRules.length > 0 && activeRules[0]) {
        ...
        if (topRule.isClosed) {
            ...
            const payload = topRule.payload;
            const hoursList = payload?.hours || [];
            const hoursText = hoursList.map((h) => `${h.open} a ${h.close}`).join(' y de ');
            ...
            if (timeContext === 'afternoon') {
                const afternoonFranja = hoursList.find((h) => parseInt(h.open.split(':')[0], 10) >= 14 || parseInt(h.close.split(':')[0], 10) >= 18);
                if (afternoonFranja) {
                ...
                const morningFranja = hoursList.find((h) => parseInt(h.open.split(':')[0], 10) < 14);
                if (morningFranja) {
...
async function seedDefaultBusinessRules(commerceId) {
    const regularRule = await prisma_1.prisma.structuredKnowledgeRule.create({
        data: {
            commerceId,
            type: 'BUSINESS_HOURS',
    ...
    const summerRule = await prisma_1.prisma.structuredKnowledgeRule.create({
        data: {
            commerceId,
            type: 'BUSINESS_HOURS',
    ...
    const holidayRule = await prisma_1.prisma.structuredKnowledgeRule.create({
        data: {
            commerceId,
...
//# sourceMappingURL=knowledge-resolver.js.map